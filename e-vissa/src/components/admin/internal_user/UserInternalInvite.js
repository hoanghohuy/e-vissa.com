...
import { Autocomplete, Checkbox, FormControl, Grid, InputLabel, MenuItem, Select } from '@mui/material';
...
export default function UserInternalInvite({
    onAdd,
    listRole,
    disabled,
    permission,
    ...
    const handleInvite = () => {
        ...
        if (listWebsite.length < 1) {
            ToastNotify('Please select a list website.');
            return;
        }
        onInvite({ email, listWebsite, role });
        setOpen(false);
    };
    ...
    const handleCreateLink = async () => {
        ...
        try {
            const listName = listWebsite.map((item) => item.name);
            console.log(listName);
            const data = await fetch(`/api/admin/users_invitation?internal=1&role=${role}&websites=${listName}`, {
                method: 'GET',
                headers: {
                        ...
                        <Grid item lg={6} xs={12} md={12} sm={12}>
                            ...
                            <FormControl fullWidth size="small">
                                <Select
                                    ...
                                    // value={role}
                                    disabled={permissionUser?.role !== 'administrator'}
